...
-- PostgreSQL: Agregacao, GROUP BY, HAVING e Window Functions
...
SELECT c.nome, COUNT(*) AS total
  FROM curso c
  JOIN turma t ON t.curso_id = c.curso_id
 WHERE t.data_inicial >= '2025-01-01'
 GROUP BY c.nome
HAVING COUNT(*) >= 1;
...
SELECT cliente_id, data_pedido, valor_total,
       LAG(valor_total)  OVER w AS anterior,
       LEAD(valor_total) OVER w AS proximo,
       ROUND(valor_total - LAG(valor_total) OVER w, 2) AS variacao
  FROM pedido
WINDOW w AS (PARTITION BY cliente_id ORDER BY data_pedido)
...
-- ============================================================
-- PARTE 9: STRING_AGG (PostgreSQL) - agregacao de string
-- ============================================================
SELECT c.nome AS curso,
       STRING_AGG(a.nome, ', ' ORDER BY a.nome) AS alunos
  FROM curso c
  JOIN turma t    ON t.curso_id = c.curso_id
  JOIN matricula m ON m.turma_id = t.turma_id
...
-- ============================================================
-- PARTE 10: WINDOW clause (PostgreSQL - reuso de definicao de janela)
-- ============================================================
SELECT nome, depto, salario,
       ROW_NUMBER() OVER w AS pos,
       AVG(salario) OVER w AS media_depto
  FROM funcionario
WINDOW w AS (PARTITION BY depto ORDER BY salario DESC);
